...
class OptimalFlowModel(BaseDensityModel):
    def __init__(self, model_path, dim=16, w=512, d=8, time_varying=True, device=None):
        self.dim = dim
        self.device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.model = MLP(dim=dim, w=w, d=d, time_varying=time_varying).to(self.device)
    ...
    @torch.no_grad()
    def generate_samples(self, num_samples: int = None, base_samples: torch.Tensor = None, steps = 100):
    ...
    def reverse(self, batch):
        ...
        t_span = torch.linspace(1, 0, 200, device=self.device)
        traj = self.node.trajectory(batch, t_span=t_span)
        return traj[-1]
    
    
    def compute_log_density(self, batch, num_div_estimates=10, steps = 100):
        """
    ...
    def compute_score(self, batch, num_div_estimates=20, steps = 100):
        """
        ...
        for t_idx in range(traj.shape[0] - 1):
            ...
            for _ in range(num_div_estimates):
                ...
                grad_output = torch.autograd.grad(
                    ...
                    inputs=traj_t,
                    grad_outputs=noise,
                    retain_graph=True,  # Only retain for this iteration
                    create_graph=False,
                    allow_unused=True
                )[0]
            ...
            # Explicit cleanup after every iteration
            del traj_t, t_input, input_with_time, divergence_estimate, noise, vec_field, grad_output
            torch.cuda.empty_cache()
        ...
        score = torch.autograd.grad(
            outputs=log_density,
            inputs=batch,
            grad_outputs=torch.ones_like(log_density),
            create_graph=False,
            retain_graph=False
        )[0]
